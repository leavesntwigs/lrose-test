c****
c******************************************************************
c**** THIS PROGRAM CALCULATES NAVIGATIONAL ERRORS:
c****
c****    D_TILT_aft, D_TILT_fore, D_ROLL, D_PITCH, D_HEADING
c****    RANGE_DELAY_aft, RANGE_DELAY_fore, D_Xwe, D_Ysn, D_Z
c****    D_VH_acft
c****
...
c****
c****
c**** THIS PROGRAM ALSO PRODUCES A "RADAR-DERIVED" SURFACE MAP
c**** (FILE "SURF_EL_*" HAS THE SAME STRUCTURE AS FILE "SURF_DTM_*")
c****
c****
c******************************************************************
c**** Author: Frank ROUX (LA, UPS-CNRS, Toulouse), March 2000  ****
c******************************************************************
c     Modified by Huaqing Cai at NCAR, April, 2010
c******************************************************************
...
c      instead of using the inlcude function above
c******************************************************************
c**** FICHIER 'mes_parametres'
c******************************************************************
c
...
! One dimensional array of DBZ, VR, SW, NCP, etc
        ...
        REAL range(MAXPORT)
        REAL ZE(MAXPORT),NCP(MAXPORT),VR(MAXPORT),SW(MAXPORT)
...
c
c******************************************************************
c**** READ THE INPUT PARAMETERS ON FILE "DATA_cns"
c******************************************************************
c
...
c
     ...
     &                         ,'-> ZSURF_CST (km) =',zsurf_cst
      if(idtmfile.eq.1)print *,' WILL READ "SURF_DTM_*" FILE :'
     &                         ,dtm_file(1:ndtmfile)
c
...
c
c******************************************************************
c**** OPEN THE OUPUT "CORNAV_EL_*" FILE #10
c******************************************************************
c
      print *,' '
...
c
c******************************************************************
c**** OPEN THE OUTPUT "SIS_EL_*" FILE #50
c******************************************************************
c
      print *,' '
...
c
  7   iend=0
      if(ifile.eq.nfile .and. lastfile .eq.1)then
          iend=2
          print *,' '
...
c
     ...
     &        +0.001*float(is_ray)+1.e-6*float(ims_ray)
      if(    time_ks-time_prev.lt.-80.
     &   .or.time_ks-tmin.lt.-80.)then
...
c CAI-STOP
	...
	print *,' HHMMSSms:',100*ihhmmss+ims_rdl
     &         ,' > HHMMSSms_max:',100*ihms_max
      ...
      if(iend.eq.2)go to 2
c
c******************************************************************
c**** CONTROL OF LAT, LON, P_ALT AND R_ALT
c******************************************************************
c
      if(    abs(lat_av).lt.0.001
     &   .or.abs(lon_av).lt.0.001
     &   .or.(     abs(p_alt_av).lt.0.001
     &        .and.abs(r_alt_av).lt.0.001))go to 1

c	print *,'P_ALT_AV= ',p_alt_av
c
c******************************************************************
c**** RADAR IDENTIFICATION THROUGH TILT_RAY (=INCL_RDL)
c****  -> AFT : IRADAR_RAY=1, IAFTFORE=-1
c****  -> FORE : IRADAR_RAY=2, IAFTFORE=+1
c**********************F********************************************
c
      tilt_ray=incl_rdl
...
c
      ...
      if(     nb_ray(iradar_ray).gt.1
     &   .and.(    (swp(iradar_ray).ne.swp_prev(iradar_ray))
     &         .or.(abs(tandrot).gt.0.2)       ) )
     &  iend=1
c!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
c!!!!      if(iend.ge.1)then
c!!!!        print *,' '
...
c
c******************************************************************
c**** MEAN VALUES FOR THE PAST SWEEP
c******************************************************************
c
...
c
c******************************************************************
c**** CONTROL PRINTS FOR THE PAST SWEEP
c******************************************************************
c
 	  print *,' '
	  print *,' '
...
c
c******************************************************************
c**** END OF THE TAPE or END OF CONSIDERED PERIOD ( IEND = 2 )
c******************************************************************
c
        if(iend.eq.2)then
	  print *,' '
...
c
c******************************************************************
c****  SUM OF INDIVIDUAL WEIGHTS, MEAN, RMS VALUES OF
c****    - DIFFERENCE OF SURFACE ALTITUDE (RADAR-DTM);
c****    - DOPPLER VELOCITY OF THE GROUND CLUTTER;
c****    - DIFFERENCE OF DOPPLER VELOCITY (RADAR-FLIGHT_LEVEL);
c******************************************************************
c
	  print *,' '
	  print *,' **********************************************'
...
c
c******************************************************************
c****  (IF SUM_WGHTS_surf+insitu > SUM_WGHTS_min)
c****   -> NAVIGATIONAL ERROS CAN BE CALCULATED
c******************************************************************
c
...
c
c******************************************************************
c**** NORMALIZED CORRELATION MATRIX BETWEEN THE NVAR VARIABLES
c******************************************************************
c
            print *,' '
...
c
c******************************************************************
c**** INITIALIZATIONS AT THE BEGINNING OF A SWEEP (if IEND=1)
c******************************************************************
c
...
c!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
c!!!!      if(      iradar_ray.eq.1
c!!!!     &    .and.nb_ray(iradar_ray)
c!!!!     &         .eq.10*(nb_ray(iradar_ray)/10) )then
c!!!!        print *,' '
...
c
c******************************************************************
c**** AIRCRAFT POSITION, (PRESSURE OR RADAR) ALTITUDE AND HEADING
c******************************************************************
c
...
c
c******************************************************************
c**** ADD TO THE MEAN PARAMETERS FOR THE CURRENT SWEEP
c******************************************************************
c
...
c******************************************************************
c
      if(    (abs(acftspd_we).lt.10..and.abs(acftspd_sn).lt.10.)
     &   .or.(abs(acftspd_we).gt.200..or.abs(acftspd_sn).gt.200.) )then
        print *,' !!!! NO_RAY:',nb_ray
...
c----------------------------------------------------------------------
      if(    (abs(wind_we).le.0..and.abs(wind_sn).le.0.)
     &   .or.(abs(wind_we).gt.100..or.abs(wind_sn).gt.100.))then
...
c!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
c!!!!
c!!!!     if(tilt_ray.lt.-15.)then
c!!!!       rota_sidelobe=rota_ray+roll_acft+6.
c!!!!     elseif(tilt_ray.gt.15.)then
c!!!!       rota_sidelobe=rota_ray+roll_acft+3.
c!!!!     endif
c!!!!      if(a_don.le.1993.and.cos(conv*rota_sidelobe).lt.0.)then
...
c
c******************************************************************
c**** DISMISS THE SPECIFIED RANGE GATES
c******************************************************************
c
      do iig=1,15
...
c
c******************************************************************
c**** RANGE GATES FOR COMPARISONS WITH FLIGHT-LEVEL (IN SITU) DATA
c******************************************************************
c
      ngates_insitu_max=-999
      if(abs(selh).lt.selhinsitu_max)then
        ig=1
        do while (     ig.lt.MAXPORT
     &            .and.dgate_corr(ig).lt.dmax_insitu)
...
c
c******************************************************************
c**** CHECK THE NCP, SW AND REFLECTIVITY VALUES
c******************************************************************
c
      ngates_max=1
      do ig=1,ngates
         ref_min=ref_min0+20.*(alog10(dgate_corr(ig))-1.)
         if(    dgate_corr(ig).lt.dmin
     &      .or.dgate_corr(ig).gt.dmax
     &      .or.ncp(ig).lt.xncp_min
     &      .or.sw(ig).gt.sw_max
     &      .or.ze(ig).lt.ref_min
     &      .or.ze(ig).gt.ref_max)then
           ze(ig)=-999.
           vr(ig)=-999.
	   vs(ig)=-999.	!Olivier
	   vl(ig)=-999.	!Olivier
...
c
c******************************************************************
c**** CHOOSE WHICH DOPPLER VELOCITY WILL BE USED (FOLLOWING ICHOICE_VDOP)
c****   -> 1:RAW(VR), 2:CORRECTED FOR VACFT(VG), 3:UNFOLDED(VU)
c******************************************************************
...
c
     ...
     &   .and.selh.lt.selh_surf
     &   .and.z_acft.gt.zacftmin_surf)then
c!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
c
c******************************************************************
c**** DETERMINE REFmax AND dREF/dD|max
c******************************************************************
c
c!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
c
c******************************************************************
c**** WEIGHT ASSOCIATED WITH THE OBTAINED SURFACE POINT
c******************************************************************
c
        if(     refmax_ray.gt.refsurf_min
     &     .and.gradrefmax_ray.gt.gradrefsurf_min)then
          if(     (d_refmax.gt.d_gradrefmax)
     &       .and.abs(z_refmax-z_gradrefmax).lt.1.)then
...
c!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
c!!!!          else
c!!!!      if( nb_ray(iradar_ray).eq.10*(nb_ray(iradar_ray)/10) )
...
c
	       if(     igsurf_ray.eq.-999
     &            .and.x_true.gt.xmin_dtm
     &            .and.x_true.lt.xmax_dtm
     &            .and.y_true.gt.ymin_dtm
...
c
c******************************************************************
c**** IF THIS SURFACE POINT IS CORRECT (if WGHTSURF_ray > 0)
...
c
c******************************************************************
c**** INTERPOLATION OF ALT_DTM(x,y) [READ ON SURF_DTM_* OR CONSTANT]
c******************************************************************
c
	  if(     xsurf_ray.gt.xmin_dtm
     &       .and.xsurf_ray.lt.xmax_dtm
     &       .and.ysurf_ray.gt.ymin_dtm
     &       .and.ysurf_ray.lt.ymax_dtm)then
...
c
c******************************************************************
c**** IF ( ABS(HSURF_RADAR-HSURF_DTM) < DHSURF_MAX ) THEN
...
c
c******************************************************************
c**** VALUES OF VAR(1->NVAR) FOR FIELD "DZ_surf"
c****  - VAR(1->6) -> [dT_aft,dT_fore,dR_aft,dR_fore,dP,dH] in DEGREES
c****  - VAR(7->11) -> [RD_aft,RD_fore,dXwe,dYsn,dZ] in HECTOMETERS
c****  - VAR(12) -> [dVH] in METER/SECOND
c******************************************************************
c
                if(iaftfore.eq.-1)then
                  if(idtiltaft.eq.1)then
...
c
c******************************************************************
c**** ADD TO XMAT_dzsurf(1->NVAR,1->NVAR) AND VECT_dzsurf(1->NVAR)
c******************************************************************
c
                do i=1,nvar
                   do j=1,nvar
...
c
c******************************************************************
c**** ADD TO COVARIANCE MATRIX FOR FIELD "DZ_surf"
c******************************************************************
c
                do i=1,nvar
...
c
c******************************************************************
c**** CASE "DZ_surf" ONLY -> D_VH CANNOT BE CALCULATED
c******************************************************************
c
...
c
c******************************************************************
c**** CASE "FLAT SURFACE" -> D_HEADING,D_XWE,D_YSN CANNOT BE OBTAINED
c******************************************************************
c
...
c
c	  if(    abs(vs_depl-vl_depl).lt.vnyq_el/2.
c     &	    .and.abs(vr(ig_refmax)-vsl_depl).lt.vnyq_el/2.)then ! Oliv
...
c
c******************************************************************
c**** VALUES OF VAR(1->NVAR) FOR FIELD "V_surf"
c****  - VAR(1->6) -> [dT_aft,dT_fore,dR_aft,dR_fore,dP,dH] in DEGREES
c****  - VAR(7->11) -> [RD_aft,RD_fore,dXwe,dYsn,dZ] in HECTOMETERS
c****  - VAR(12) -> [dVH] in METER/SECOND
c******************************************************************
...
c
c******************************************************************
c**** ADD TO XMAT_vsurf(1->NVAR,1->NVAR) AND VECT_vsurf(1->NVAR)
c******************************************************************
c
                    do i=1,nvar
                       do j=1,nvar
...
c
c******************************************************************
c**** ADD TO COVARIANCE MATRIX FOR FIELD "VDOP_surf"
c******************************************************************
c
                    do i=1,nvar
...
c
c******************************************************************
c**** CASE "VDOP_surf" and/or "DVDOP_insitu" ONLY :
c**** -> RGE-DLY_aft,RGE-DLY_aft,D_XWE,D_YSN,D_ZACFT CANNOT BE CALCULATED
c******************************************************************
c
...
c
     ...
     &                     =vdopsurf_ray
c
	          else  !!  of  !! if(abs(vdopsurf_ray).lt.vdopsurf_max) !!
	            ndismiss_vdopsurf(iradar_ray)
...
c
c!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
c!!!!          else !!  of  !!  if(xsurf_ray.gt.xmin_dtm ... )  !!
c!!!!            if( nb_ray(iradar_ray).eq.10*(nb_ray(iradar_ray)/10) )then
...
c******************************************************************
c**** CASE "DVDOP_insitu"
c**** (if D<DMAX_insitu and ||sin(ELEV_HOR)||<0.1)
c******************************************************************
c
      if(kdvinsitu.eq.1.and.ngates_insitu_max.gt.1)then
c
c******************************************************************
c**** CONTROL CONTINUITY ALONG THE RAY ( if ICTRL_CONTRAY=1 )
c**** DISMISS VDOP IF |VDOP-VDOP_PREV|>dVDOP_MAX AFTER UNFOLDING
c******************************************************************
c
...
c!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      do ig=1,ngates_insitu_max
         if(     ze(ig).gt.-900.
     &        .and.vdop_corr(ig).gt.-900.)itest=1
      enddo
      if(     itest.eq.1
     &   .and.nb_ray(iradar_ray).eq.5*(nb_ray(iradar_ray)/5))then
        print *,' '
        print *,' ',1000*ihhmmss+ims_ray
     &         ,' IRADAR:',iradar_ray
     &         ,' NO_RAY:',nb_ray(iradar_ray)
...
c
c******************************************************************
c**** ADD WEIGHTS AND DVDOP_insitu
c******************************************************************
c
...
c!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
c!!!!      if( nb_ray(iradar_ray).eq.5*(nb_ray(iradar_ray)/5) )then
...
c
c******************************************************************
c**** VALUES OF VAR(1->NVAR) FOR FIELD "DV_insitu"
c****  - VAR(1->6) -> [dT_aft,dT_fore,dR_aft,dR_fore,dP,dH] in DEGREES
c****  - VAR(7->11) -> [RD_aft,RD_fore,dXwe,dYsn,dZ] in HECTOMETERS
c****  - VAR(12) -> [dVH] in METER/SECOND
c******************************************************************
...
c
c******************************************************************
c**** ADD TO XMAT_dvinsitu(1->NVAR,1->NVAR) AND VECT_dvinsitu(1->NVAR)
c******************************************************************
c
               do i=1,nvar
   	          do j=1,nvar
...
c
c******************************************************************
c**** ADD TO COVARIANCE MATRIX FOR FIELD "DVDOP_insitu"
c******************************************************************
c
               do i=1,nvar
...
c****************************************************************************
c
c**** INVERSION OF MATRIX (NVAR,NVAR)
c
c****************************************************************************
c
