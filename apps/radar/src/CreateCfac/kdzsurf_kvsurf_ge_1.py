            ...
            if dgate_corr[ig] <= distmax:
                ...
                if ze[ig] > -900.:
                    ...
                    if ig > 0 and ze[ig - 1] > -900.:
                        print('4')
                        gradref = (ze[ig] - ze[ig - 1]) / (d_ig - dgate_corr[ig - 1])  # must be > 20
                        print('gradref = ', gradref)
                        if gradref > gradrefmax_ray:
                            print('5: new gradrefmax_ray: ', gradref)
...
# autoconverted end
# 
#   these variables are set by the above loop:
#       dhor_ig
#       dhor_gradrefmax
#       dhor_refmax
#       dver_ig
...
#******************************************************************
#
        print('before line 159: refmax_ray = ', refmax_ray, ' refsurf_min = ', refsurf_min) 
        print(' gradrefmax_ray = ', gradrefmax_ray, ' gradrefsurf_min = ', gradrefsurf_min) 
        print(' d_refmax = ', d_refmax, ' d_gradrefmax = ', refsurf_min)
        print(' abs(z_refmax-z_gradrefmax) = ', abs(z_refmax-z_gradrefmax))
        if (refmax_ray > refsurf_min
            ...
            if(     (d_refmax > d_gradrefmax)
                ...
                wghtsurf_ray=np.sqrt(wght_ref*wght_grad)
                dsurf_ray=d_refmax
                hsurf_ray=z_refmax
...
#******************************************************************
#
        print('before line 188: hsurf_ray = ', hsurf_ray, ' wghtsurf_ray = ', wghtsurf_ray)
...
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      #******************************************************************
      #
                    # if(kvsurf == 1 and acftspd_hor > 0.):   # NOTE: kvsurf = 1 ALWAYS   # E
                    if(acftspd_hor > 0.):                       # E
      #
                        if(vdop_corr[ig_refmax] > -900. or isim == 1):
                            vdopsurf_ray=vdop_corr[ig_refmax]
                            # the nb's are never used
...
#
    return n_dzsurf, xmat_vsurf, vect_vsurf, corr_var
